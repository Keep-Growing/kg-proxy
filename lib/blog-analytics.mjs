...
export function blogAnalyticsRuntime() {
  ...
  document.addEventListener('click', event => {
    ...
    const offers = {
      ...
      '/sales-pilot': 'sales_pilot', '/pulse-formation': 'pulse_formation',
      '/atelier-ia-generative': 'atelier_ia', '/cabinets-experts': 'cabinets_experts'
    };
...
// Vercel Web Analytics + Speed Insights, same tags as the static site pages.
// Cookie-free, so they do not depend on the GA4 consent banner.
const VERCEL_TAGS = '<script defer src="/_vercel/insights/script.js"></script>' +
  '<script defer src="/_vercel/speed-insights/script.js"></script>';
export function injectVercelInsights(html) {
  if (html.includes('/_vercel/insights/script.js')) return html;
  if (!/<\/body>/i.test(html)) return html;
  return html.replace(/<\/body>(?![\s\S]*<\/body>)/i, VERCEL_TAGS + '</body>');
}
